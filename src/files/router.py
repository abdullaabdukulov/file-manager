...
from src.files.service import delete_file, download_file, get_file, list_files, upload_file
...
async def upload_file_endpoint(
        ...
        visibility: Visibility = Query(..., description="Visibility: PRIVATE, DEPARTMENT, or PUBLIC"),
...
async def download_file_endpoint(file_id: UUID, current_user: dict = Depends(get_current_user)):
...
async def delete_file_endpoint(file_id: UUID, current_user: dict = Depends(get_current_user)):
...
async def list_files_endpoint(
        department_id: UUID | None = Query(None, description="Optional department ID to filter files"),
        current_user: dict = Depends(get_current_user)
):
